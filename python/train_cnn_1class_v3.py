...
"""
================================================================================
CNN Training Script v3 - Binary COPD with 5-Fold GroupKFold + Progress Reporting
================================================================================

Same logic as train_cnn_1class_v2.py, with added:
- Clear Fold X/Y and Epoch A/B progress
- Per-epoch stats: Train/Val Loss, Train/Val Acc, Val F1, best checkpoint, early-stop counter
- Fold summary banner (train/val/test sizes)

Usage:
    python train_cnn_1class_v3.py --data_path /path/to/ICBHI --output_dir ./output_copd_v3
"""
...
# ==============================================================================
# PROGRESS: print epoch stats and fold banner
# ==============================================================================
...
def print_epoch_progress(fold_id: int, n_folds: int, phase: str, epoch: int, total_epochs: int,
                         ...
                         val_f1: float, is_best: bool, patience_counter: int):
    """One-line per-epoch: Fold | Phase | Epoch | Loss/Acc/F1 | best? | patience."""
    best_mark = " [BEST]" if is_best else ""
    pat_str = f" | patience={patience_counter}/{EARLY_STOP_PATIENCE}" if phase == "Phase 2" else ""
    print(f"  Fold {fold_id+1}/{n_folds} | {phase} | Epoch {epoch+1:3d}/{total_epochs} | "
          f"Train Loss: {train_loss:.4f} | Val Loss: {val_loss:.4f} | "
          f"Train Acc: {train_acc*100:5.2f}% | Val Acc: {val_acc*100:5.2f}% | Val F1: {val_f1*100:5.2f}%{best_mark}{pat_str}")


def print_fold_banner(fold_id: int, n_folds: int, n_train: int, n_val: int, n_test: int):
    """Banner at start of each fold with sample counts."""
    print("\n" + "="*70)
...
class WaveletTransform:
    def __init__(self, wavelet=WAVELET_NAME, num_scales=NUM_SCALES,
                 sample_rate=TARGET_SR, freq_range=FREQUENCY_RANGE, output_size=IMG_SIZE):
...
class ICBHIBinaryDatasetV2(Dataset):
    def __init__(self, file_list, labels, patient_ids, data_path, transform=None,
                 wavelet_transform=None, augment=False, oversample_minority=False, spec_augment=False):
...
def compute_metrics(y_true, y_pred):
    ...
    return {'accuracy': acc, 'f1_macro': f1_macro, 'sensitivity': sensitivity, 'specificity': specificity, 'confusion_matrix': cm}
...
# ==============================================================================
# TRAIN ONE FOLD (with per-epoch progress)
# ==============================================================================
...
def train_fold(fold_id: int, n_folds: int, train_idx, val_idx, test_idx,
    ...
    for epoch in range(phase1_epochs):
        ...
        print_epoch_progress(fold_id, n_folds, "Phase 1", epoch, total_epochs,
                             train_loss, val_loss, train_acc, val_acc, val_f1, is_best, patience_counter)
...
def plot_aggregate_curves(all_histories: List[Dict], save_path: str):
    ...
    fig, axes = plt.subplots(1, 2, figsize=(12, 4))
    axes[0].plot(epochs, np.nanmean(train_loss, axis=0), 'b-', label='Train (mean)', lw=1.5)
    axes[0].fill_between(epochs, np.nanmean(train_loss, axis=0) - np.nanstd(train_loss, axis=0),
                         np.nanmean(train_loss, axis=0) + np.nanstd(train_loss, axis=0), alpha=0.2, color='b')
    axes[0].plot(epochs, np.nanmean(val_loss, axis=0), 'r-', label='Val (mean)', lw=1.5)
    axes[0].fill_between(epochs, np.nanmean(val_loss, axis=0) - np.nanstd(val_loss, axis=0),
                         np.nanmean(val_loss, axis=0) + np.nanstd(val_loss, axis=0), alpha=0.2, color='r')
    axes[0].set_xlabel('Epoch')
    axes[0].set_ylabel('Loss')
    axes[0].set_title('Average Loss (Mean ± Std)')
    axes[0].legend()
    axes[0].grid(True, alpha=0.3)
    axes[1].plot(epochs, np.nanmean(train_acc, axis=0), 'b-', label='Train (mean)', lw=1.5)
    axes[1].fill_between(epochs, np.nanmean(train_acc, axis=0) - np.nanstd(train_acc, axis=0),
                         np.nanmean(train_acc, axis=0) + np.nanstd(train_acc, axis=0), alpha=0.2, color='b')
    axes[1].plot(epochs, np.nanmean(val_acc, axis=0), 'r-', label='Val (mean)', lw=1.5)
    axes[1].fill_between(epochs, np.nanmean(val_acc, axis=0) - np.nanstd(val_acc, axis=0),
                         np.nanmean(val_acc, axis=0) + np.nanstd(val_acc, axis=0), alpha=0.2, color='r')
    axes[1].set_xlabel('Epoch')
    axes[1].set_ylabel('Accuracy')
...
def main(args):
    print("\n" + "="*70)
    print("  COPD Binary v3 - 5-Fold GroupKFold (with progress reporting)")
    print("="*70)
    print(f"  Device: {DEVICE}  |  Data: {args.data_path}")
    print(f"  Folds: {N_FOLDS}  |  Epochs: {args.epochs}  |  Phase2 LR: {PHASE2_LR}  |  Early-stop patience: {EARLY_STOP_PATIENCE}")
    print("="*70 + "\n")
    ...
    for fold_id in range(N_FOLDS):
        ...
        print(f"  >> Fold {fold_id} done | Test Acc: {test_metrics['accuracy']*100:.2f}%  F1: {test_metrics['f1_macro']*100:.2f}%  Sens: {test_metrics['sensitivity']*100:.2f}%  Spec: {test_metrics['specificity']*100:.2f}%")
    ...
    with open(output_dir / "cv_metrics.json", 'w') as f:
        json.dump({
            ...
            'specificity_mean': float(np.mean(spec)), 'specificity_std': float(np.std(spec)),
            'best_f1_fold': int(best_f1_fold),
            'per_fold': [{'accuracy': m['accuracy'], 'f1_macro': m['f1_macro'],
                         'sensitivity': m['sensitivity'], 'specificity': m['specificity']} for m in all_test_metrics],
        }, f, indent=2)
    print(f"\nAll outputs in {output_dir}. Done.")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="COPD Binary v3 - 5-Fold + Progress")
    parser.add_argument("--data_path", type=str,
                        default="/home/iec/Parallel_Computing_on_FPGA/data/samples/ICBHI_final_database")
    parser.add_argument("--output_dir", type=str, default="./output_copd_v3")
