...
from .meta_description_ver2 import generate_meta_description
...
def get_ensemble_prompt(
    ...
    def augment(x):
        # Flip some randomly when training
        if training and random.random() < p_flip:
            return random.choice(labels)
        return x

    items = []
    for idx, d in enumerate(predictions):
        label = augment(labels[d])
        if training and random.random() < p_mask:
            continue
        items.append(f"Expert {idx + 1}: \"{label}\"")
...
def get_mean_soft_label(
        logits,
        label=None,
        use_softmax=True,
        ):
    logits = torch.tensor(logits).mean(axis=0)
    if use_softmax:
        return torch.nn.functional.softmax(logits)
    return logits
...
def get_onehot_label(
    ...
    logits = torch.tensor(logits)
    dtype = logits.dtype
    logits = torch.nn.functional.one_hot(torch.tensor(label), logits.shape[1]).to(dtype)

    if variance is not None:
        variance = torch.tensor(variance)
        std_dev = torch.sqrt(variance)
        if mean is None:
            mean = [0 for _ in logits]
        mean = torch.tensor(mean).to(dtype)
        noise_vector = torch.normal(mean, std_dev)
        logits += noise_vector
    return logits
