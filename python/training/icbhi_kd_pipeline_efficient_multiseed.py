#!/usr/bin/env python3
"""
ICBHI 2017 official cycle-level KD pipeline — Efficient multiseed CNN variant.

This is a complete end-to-end KD experiment pipeline using the shared strategy with
compute-friendly defaults for running more seeds quickly:
  - CNN-only teacher ensemble (no EfficientNet dependency by default)
  - log-mel input for lower preprocessing/memory cost
  - compact DS-CNN-Res-SE student
  - reliability-weighted KD + binary auxiliary loss
  - final threshold tuning and W&B tracking
"""
...
def parse_args():
    ...
    if args.pipeline_name == "icbhi_kd_multiview_ensemble":
        args.pipeline_name = "icbhi_kd_efficient_multiseed"
    if args.teacher_arches == "resnet_cnn,resnet_crnn,efficientnet_b0":
        args.teacher_arches = "resnet_cnn"
    if args.seeds == "1,2,3":
        args.seeds = "1,2,3,4,5"
    args.input_view = "logmel"
    if args.student_width == 1.0:
        args.student_width = 0.85
    if args.epochs_teacher == 80:
        args.epochs_teacher = 60
    if args.epochs_student == 120:
        args.epochs_student = 90
    if args.f_max == 4000.0:
        args.f_max = 2500.0
    if args.temperature == 4.0:
        args.temperature = 3.0
    if args.hard_weight == 0.35:
        args.hard_weight = 0.40
