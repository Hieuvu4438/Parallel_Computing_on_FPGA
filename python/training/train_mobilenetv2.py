...
"""
================================================================================
MobileNetV2 Training Script — 3-Class Respiratory Sound Classification
================================================================================
OPTIMIZED version applying all anti-overfitting strategies:

  1. Focal Loss + Label Smoothing (handles class imbalance + overconfidence)
  2. Gradual Unfreezing (4-phase: head → last 4 blocks → last 8 → all)
  3. Discriminative Learning Rates (lower LR for pretrained layers)
  4. MixUp Training (alpha=0.3 — creates virtual examples, reduces memorization)
  5. Enhanced Audio Augmentation (SNR noise, shift, gain, polarity, time-stretch)
  6. Enhanced SpecAugment (freq_mask=30, time_mask=40, p=0.7)
  7. Early Stopping on Macro F1 (not val_loss)
  8. Gradient Clipping (max_norm=1.0)
  9. Higher Weight Decay (0.05)
  10. Augment ALL classes equally at 70% probability
  11. Stronger classifier head with BatchNorm

Pipeline: Raw WAV → Resample 4kHz → BPF → CWT Morlet Spectrogram → MobileNetV2

Usage:
    python train_mobilenetv2.py \\
        --data_path data/combined/audio \\
        --labels_csv data/combined/labels.csv \\
        --output_dir artifacts/training/mobilenetv2_3class_raw

    # Quick test (1 fold, 2 epochs):
    python train_mobilenetv2.py --dry_run --epochs 2
"""
...
# --- Training Schedule (Gradual Unfreezing) ---
PHASE1_EPOCHS = 15          # Head-only training
PHASE2A_EPOCHS = 20         # Last 4 InvertedResidual blocks unfrozen
PHASE2B_EPOCHS = 20         # Last 8 blocks unfrozen
...
class Logger:
    def __init__(self, log_path: str):
        ...
        with open(self.log_path, 'w') as f:
            f.write("=" * 70 + "\n")
            f.write("  TRAINING LOG: MobileNetV2 Optimized Pipeline (3 Classes)\n")
            f.write("=" * 70 + "\n")
...
class AudioAugmenter:
    ...
    Includes: SNR-based noise, time-shift, gain, polarity inversion, time-stretch.
    """
...
class SpecAugment:
    ...
    def __call__(self, spec: torch.Tensor) -> torch.Tensor:
        ...
        if HAS_TORCHAUDIO and self.freq_mask is not None:
            ...
            # Second pass for stronger augmentation
            if np.random.random() < 0.3:
                spec = self.freq_mask(spec)
                spec = self.time_mask(spec)
            return spec
...
class AudioSpectrogramDataset(Dataset):
    ...
    Augmentation is applied to ALL classes equally (not just minority).
    """
    ...
    def __getitem__(self, idx):
        ...
        # Audio augmentation — ALL classes equally
        if self.augment and self.augmenter:
            audio = self.augmenter(audio)

        # CWT Spectrogram
        spec_image = self.wavelet_transform.to_image(audio)
        spec_image = np.stack([spec_image] * 3, axis=0)  # RGB channels for pretrained model
...
# ==============================================================================
# MODEL ARCHITECTURE: MobileNetV2 with Improved Head
# ==============================================================================
class MobileNetV2Classifier(nn.Module):
    """
    MobileNetV2 with stronger multi-layer classifier head + BatchNorm.
    MobileNetV2 backbone has 19 feature blocks (features[0]..features[18]).
    Output: 1280 channels after pooling.
    """
    def __init__(self, num_classes=3, pretrained=True, head_dropout=0.5):
        ...
        # Stronger classifier head with BatchNorm layers
    ...
    def unfreeze_last_n_blocks(self, n: int):
        """
        Gradually unfreeze the last n blocks of MobileNetV2 backbone.
        MobileNetV2 has 19 blocks: features[0]..features[18].
        - features[0]: initial conv
        - features[1..17]: 17 InvertedResidual blocks
        - features[18]: final conv (1x1)
        """
        for p in self.backbone.features.parameters():
            p.requires_grad = False
        total_blocks = len(self.backbone.features)  # 19
...
# ==============================================================================
# TRAINING FOLD — GRADUAL UNFREEZING (4 PHASES)
# ==============================================================================
def train_fold(fold_id: int, n_folds: int, train_idx, val_idx, test_idx,
    ...
    train_files, train_labels, train_pids = oversample_multiclass(
    ...
    # Helper to run one training phase
    def run_phase(phase_name, epoch_start, epoch_end, use_mixup,
                  scheduler, is_batch_scheduler=False):
        nonlocal best_val_f1, patience_counter

        for epoch in range(epoch_start, epoch_end):
            bs = scheduler if is_batch_scheduler else None
            train_loss, train_acc = train_one_epoch(
                model, train_loader, criterion, optimizer, DEVICE,
                use_mixup=use_mixup, batch_scheduler=bs
            )
            ...
            # Epoch-level scheduler step (for CosineAnnealing)
            ...
            pat_str = f" | patience={patience_counter}/{EARLY_STOP_PATIENCE}" if epoch_start >= p1_end else ""
            log.print(
                f"  Fold {fold_id+1}/{n_folds} | {phase_name} | "
            ...
            if epoch_start >= p1_end and patience_counter >= EARLY_STOP_PATIENCE:
                log.print(f"  >> Early stopping at epoch {epoch+1} ({phase_name}) "
                          f"due to F1 patience.")
    ...
    run_phase("Phase 1", 0, p1_end, use_mixup=False,
              scheduler=scheduler, is_batch_scheduler=True)

    # ===================== PHASE 2a: Last 4 blocks =====================
    ...
    scheduler = optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=PHASE2A_EPOCHS, eta_min=1e-7
    )
    patience_counter = 0

    log.print(f"  --- Phase 2a: Last 4 Blocks Unfrozen (Ep {p1_end+1}-{p2a_end}) | "
              f"LR bb={PHASE2A_LR_BACKBONE}, head={PHASE2A_LR_HEAD} ---")
    stopped = run_phase("Phase 2a", p1_end, p2a_end, use_mixup=True,
                        scheduler=scheduler, is_batch_scheduler=False)

    # ===================== PHASE 2b: Last 8 blocks =====================
    if not stopped:
        model.unfreeze_last_n_blocks(8)
        param_groups = model.get_param_groups(PHASE2B_LR_BACKBONE, PHASE2B_LR_HEAD)
        optimizer = optim.AdamW(param_groups, weight_decay=WEIGHT_DECAY)
        scheduler = optim.lr_scheduler.CosineAnnealingLR(
            optimizer, T_max=PHASE2B_EPOCHS, eta_min=1e-7
        )
        patience_counter = 0

        log.print(f"  --- Phase 2b: Last 8 Blocks Unfrozen (Ep {p2a_end+1}-{p2b_end}) | "
                  f"LR bb={PHASE2B_LR_BACKBONE}, head={PHASE2B_LR_HEAD} ---")
        stopped = run_phase("Phase 2b", p2a_end, p2b_end, use_mixup=True,
                            scheduler=scheduler, is_batch_scheduler=False)
    ...
    if not stopped:
        ...
        log.print(f"  --- Phase 2c: All Unfrozen (Ep {p2b_end+1}-{p2c_end}) | "
                  f"LR bb={PHASE2C_LR_BACKBONE}, head={PHASE2C_LR_HEAD} ---")
        run_phase("Phase 2c", p2b_end, p2c_end, use_mixup=True,
                  scheduler=scheduler, is_batch_scheduler=False)
...
def plot_fold_history(history: Dict, save_path: str, fold_id: int):
    ...
    for ax in axes:
        for boundary, label in [(PHASE1_EPOCHS, '2a'),
                                 (PHASE1_EPOCHS + PHASE2A_EPOCHS, '2b'),
                                 (PHASE1_EPOCHS + PHASE2A_EPOCHS + PHASE2B_EPOCHS, '2c')]:
            if boundary < len(list(epochs)):
...
def main(args):
    ...
    log.print(f"Model: MobileNetV2 | Loss: Focal (γ={FOCAL_GAMMA}) + "
              f"LabelSmoothing({LABEL_SMOOTHING})")
    log.print(f"MixUp α={MIXUP_ALPHA} | Weight Decay={WEIGHT_DECAY} | "
              f"Grad Clip={GRADIENT_CLIP_NORM}")
    log.print(f"Gradual Unfreezing: P1={PHASE1_EPOCHS}ep, P2a={PHASE2A_EPOCHS}ep, "
              f"P2b={PHASE2B_EPOCHS}ep, P2c={PHASE2C_EPOCHS}ep")
    ...
    # --- Export Best Model ---
    final_pth = output_dir / "mobilenetv2_3class_raw_best.pth"
    onnx_path = output_dir / "mobilenetv2_3class_raw_best.onnx"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="MobileNetV2 Optimized Training — 3-Class Respiratory Sound"
    )
    ...
    parser.add_argument("--epochs", type=int, default=100,
                        help="Max epochs (may be less due to gradual unfreezing schedule)")
    ...
    if args.output_dir is None:
        args.output_dir = str(Path(args.artifact_root) / "mobilenetv2_3class_raw")
    main(args)
