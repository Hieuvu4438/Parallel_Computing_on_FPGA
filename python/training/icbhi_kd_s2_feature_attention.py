...
def get_feature_channels(student_arch, teacher_arches, nc, in_ch):
    ...
    # Teacher feature channels (verified by hook inspection)
    t_ch_map = {
...
def train_student_with_feature_kd(args, splits, stats, device, output_dir):
    ...
    # Curated ensemble: select top-K teachers by val ICBHI
    curated_k = getattr(args, 'curated_k', 0)
    if curated_k > 0 and curated_k < val_logits.shape[0]:
        top_k = base.select_top_teachers(val_logits, splits["val"], args.num_classes, k=curated_k)
        val_logits = val_logits[top_k]
        train_logits = train_logits[top_k]
        teacher_names = [teacher_names[i] for i in top_k]
        print(f"  Curated ensemble: using {curated_k} teachers: {teacher_names}")
    ...
    proj_dim = 64
    # Separate projectors: student features and teacher features have different channel dims
    student_projector = FeatureProjector([s_ch[0], s_ch[1], s_ch[2]], proj_dim).to(device)
    # Per-architecture teacher projectors (fixes channel mismatch bug)
    t_ch_map = {
        "resnet_cnn": [32, 64, 160],
        "resnet_crnn": [64, 96, 128],
        "efficientnet_b0": [40, 112, 320],
    }
    unique_archs = list(dict.fromkeys(teacher_arches))  # preserve order, deduplicate
    ...
    # Keep a reference for backward compat in optimizer
    teacher_projector = teacher_projectors[unique_archs[0]]

    # Adversarial discriminator (3 levels × proj_dim × 8 × 8)
    disc_feat_dim = proj_dim * 8 * 8 * 3
    ...
    student_params = list(student.parameters()) + list(student_projector.parameters()) + teacher_proj_params
    use_sam = getattr(args, 'use_sam', False)
    if use_sam:
        opt_s = base.make_sam_optimizer(student_params, lr=args.lr_student,
    ...
    for arch in teacher_arches:
        for seed in base.parse_int_csv(args.seeds):
            ...
            if ckpt_path.exists():
                ...
                # Use checkpoint's num_classes to avoid size mismatch
                ckpt_nc = ckpt.get("args", {}).get("num_classes", args.num_classes)
                ...
                print(f"  Loaded teacher: {arch} seed={seed} (nc={ckpt_nc})", flush=True)
    ...
    for epoch in range(1, args.epochs_student + 1):
        ...
        for x, y, _, tprob in train_loader:
            ...
            s_feats_projected = student_projector([s_feats.get("feat_early", torch.zeros(1)),
            ...
            # --- Logit KD loss (with optional label smoothing) ---
            kd_smoothing = getattr(args, 'kd_smoothing', 0.0)
            if kd_smoothing > 0:
                kd_loss = base.smoothed_kd_loss(logits, tprob, args.temperature, smoothing=kd_smoothing)
            else:
            ...
            if teacher_models and args.feat_weight > 0:
                ...
                # Project teacher features using per-arch projector
                t_arch = t_model.arch_name
                t_proj = teacher_projectors[t_arch] if t_arch in teacher_projectors else teacher_projectors[unique_archs[0]]
            ...
            if teacher_models and args.adv_weight > 0 and epoch > 5:
                # Get teacher features (use same cycled teacher as feature KD)
                with torch.no_grad():
                    t_model_adv = teacher_models[epoch % len(teacher_models)]
                    _, t_feats = t_model_adv(x)
                    t_arch_adv = t_model_adv.arch_name
                    t_proj_adv = teacher_projectors[t_arch_adv] if t_arch_adv in teacher_projectors else teacher_projectors[unique_archs[0]]
                    t_proj = t_proj_adv([t_feats.get(k, torch.zeros(1)) for k in ["feat_early", "feat_mid", "feat_late"]])
            ...
            if use_sam:
                ...
                # SAM second step (recompute loss at perturbed weights)
                logits2, s_feats2 = student(x)
                kd_loss2 = -(tprob * F.log_softmax(logits2 / args.temperature, dim=1)).sum(dim=1).mean() * (args.temperature ** 2)
                hard_loss2 = hard(logits2, y)
                loss2 = args.hard_weight * hard_loss2 + args.kd_weight * kd_loss2
                loss2.backward()
                clip_all_grads()
...
def parse_args():
    ...
    if args.pipeline_name == "icbhi_kd_multiview_ensemble":
        args.pipeline_name = f"icbhi_kd_s2_featattn_{args.num_classes}class"
    ...
    # Loss rebalancing for feature KD - use CLI values
